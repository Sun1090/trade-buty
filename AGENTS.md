...
- **Remote topic branches are temporary PR transport, not persistent storage.** Do not push `codex/*`, `feat/*`, or any other topic branch merely for backup/checkpoints. Push one only when opening or updating its PR. Delete a remote branch only **after its PR has actually merged** — closing a PR by deleting its head branch strands the commits (this happened: four PRs closed unmerged, 19 commits survived only on a local `main`, recovered later as PR #105). GitHub auto-deletes head branches on merge, so manual deletion is usually unnecessary. Before starting another branch, audit open PRs and `git log origin/main..HEAD` on local branches; finish/merge viable work and remove branches already merged.
- Never leave work committed only to a local branch: if `main` is protected, land it through a PR even when the queue is long.
