...
> 技术侧已就绪：sitemap（430 URL，其中知识库 418）、robots、OG 卡片、双语路由、全站 SSG。
...
- [自动] ✅ 旧链接或拼写错误落入未知章节/课程、根级 404、搜索零结果时，均提供当前语言下的搜索与学习路线出口（R13.18）

## 2. 内容分发（冷启动主渠道）

知识库 182 篇 = 天然的选题库。每个渠道的打法：
...
- [自动] ✅ 已定义本地 debug 转化事件契约（R13.19）：分享下载/预览/复制、落地页 CTA、邀请展示/关闭/清除；R13.20 隐私审计锁定 console-only，接入任何远端遥测前必须重做供应商/同意/保留期评审
