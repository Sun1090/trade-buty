...
> 自动生成于 2026-09-21（`npm run check:test-clock-hygiene`），勿手改。
...
| uncontrolled-timer | src/components/replay-trainer.test.tsx | 279 | `await new Promise((r) => setTimeout(r, 700));` |
| uncontrolled-timer | src/lib/reading-time.test.ts | 14 | `const flushAsync = () => new Promise((resolve) => setTimeout(resolve, 0));` |
| uncontrolled-timer | src/lib/sync-layer-failure-queue.test.ts | 59 | `await new Promise((resolve) => setTimeout(resolve, 0));` |
