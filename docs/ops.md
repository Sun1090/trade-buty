...
历史上真出过问题的入口。下表第一列就是脚本打印的那个名字（`scripts/prod-smoke.test.mjs` 逐字比对两边，
改名或加一条而没同步这张表即红）：

| 断言 | 为什么在清单上 |
|---|---|
| `GET /zh → 200 且含风险提示` | 中文首页：它挂了读起来像整站下线 |
| `GET /en → 200 且含风险提示` | 英文首页：双语路由的另一侧只在这条上验证 |
| `GET /zh/knowledge/getting-started → 200 且含风险提示` | 课程页与课文页由知识库 markdown 生成，构建绿不代表这一层有内容 |
| `GET /zh/knowledge/getting-started/candlestick-basics → 200 且含风险提示` | 课文页：内容红线（`⚠️` 风险提示块）落地的最深处，历史上游客判定与风险块都在这两处出过问题 |
| `GET /sitemap.xml → 200 且是 urlset` | 抓取入口退化时页面本身看不出来 |
| `GET /robots.txt → 200 且指向 sitemap` | 同上：robots 丢掉 `Sitemap:` 行，索引会慢慢掉 |
| `GET /zh/changelog → 含最新发布版本（部署跟上 main 的探针）` | 「合并 ≠ 上线」：Vercel 配额限流会让生产停在旧构建（0.7.2 就停过一次） |
| `GET /share/streak/<合法载荷> → 200 且含风险提示（R14.7）` | R14.7：分享落地页曾经漏掉风险行 |
| `GET /api/auth/session 匿名 → 200 {"user":null}（#107 游客判定回归）` | PR #107：游客被判 500 的回归 |
| `POST /api/ai/chat 游客：护栏路径 200+X-Refused，模型路径要么答要么明确没测` | 拆成两段是因为一句「AI 502」分不清站内还是站外：护栏在调用上游**之前**就返回，它 200 且带 `X-Refused` 就证明函数活着、部署没落后、内容红线还在，此时模型路径 5xx 只可能是上游配置或出口。模型路径单独等 `AI_MODEL_TIMEOUT_MS`（90s）——生产实测上游不通时这一跳要挂约 68s 才回 502，用全站那 20s 只会得到「我们超时了」 |
