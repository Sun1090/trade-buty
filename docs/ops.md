...
| `npm run check:dead-copy` | 字典死键巡检，两路口径（R16.21 + R16.78）。① `src/lib/i18n*.ts` 词条若在 `src/`（字典文件自身除外）、`e2e/`、`scripts/` 的非测试代码里没有任何引用点即失败。② 组件自己声明的字典接口（名字带 `Dict`、字段全是 `string`/`string[]` 的那种）若某字段在**声明它的本文件**里从头到尾没出现过即失败——第一目对这类是瞎的：词条在页面有装配点（`error: t.chapter.aiSummaryError`）就算「被使用」，可接住它的组件一次都没读过它，改字典的人看着装配点真心以为改到了界面（R16.77 的 `error` 是第一例）。第二路保守到「字段名以任意形态出现在声明块之外都算读过」，因为 `replay-trainer.tsx` 用 `{ labelKey: "difficultyNew" }` + `dict[d.labelKey]` 间接读；判定逐文件，别处读过不算。两路都从同一个 `scripts/dead-copy-budget.json` 取预算，当前均为 **0**，缺任一键即判失败（拿到 `undefined` 的比较会永远通过）；提取量低于下限同样失败（否则台账会变成空转） | 直接删掉没人读的词条（连同假字典里的引用）；组件字段要么读掉，要么把接口字段和页面装配点一起删。确需保留的在 PR 里写明理由并上调预算，不得为了变绿而加 `// eslint-disable` 式的豁免 |
| `npm run check:localized-labels` | 写死单一语言的界面文案巡检（R16.50 + R16.51）：`src/**/*.tsx`（测试文件除外）里查两类——`title` / `aria-label` / `alt` / `placeholder` / `label` 的字面量值，以及 `>…<` 之间的裸 JSX 文本节点；含中日韩文字即失败。前者会被屏幕阅读器和提示气泡念出来（`read-aloud` 的 `title="语速"` 是第一例），后者是眼睛直接看到的按钮字（复习页一次有七条界面字没问语言，其中五处是裸 JSX 文本、另两条藏在表达式里，是第二例）。走表达式的按语言写法 `{locale === "en" ? "Back" : "返回"}` 不在形状里，所以不误伤。豁免三处：根级 `not-found.tsx` / `error.tsx`（整页在 `[locale]` 之外，语言无从判定，归 R16.41）与分享卡的降级图（payload 解码失败时访客语言无从判定，正常的三张卡和课程页 OG 卡都已按 locale 出文案，这一张要不要中英并列归 R16.52）；豁免清单由用例钉住当前条数，往里加一条就等于放弃一处检查 | 改成 `{locale === "en" ? … : …}` 或走 `src/lib/i18n.ts` 字典，并在用例里按语言断言；把英文写死在同一处它认不出来（判据是 CJK 字符），那半边靠按 locale 的断言补 |
| `npm run check:report-freshness` | 重算型报告新鲜度核对（R16.26）：报告清单由「谁调用了幂等写入器 `writeReport`」推导（当前 17 份，见 `scripts/report-freshness-lib.mjs`），入库版本必须等于当场重算结果——被门禁改写过的就是过期版本，推导数为 0 也判失败而不是「无报告可核对」的假绿。按日追加的历史快照（`kb:translation-status`、`kb:diff`）不走幂等写入器，天然不在范围内 | 排在 CI 所有报告步骤之后；命中就把重算结果一起提交（`npm run check:<对应门禁>` 后 `git add docs/…`）。若某份报告确实该每天变，说明它不该走 `writeReport`，把它挪进历史快照通道 |
