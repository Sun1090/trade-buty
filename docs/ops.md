...
| `npm run check:localized-labels` | 写死单一语言的文案属性巡检（R16.50）：`src/**/*.tsx`（测试文件除外）里 `title` / `aria-label` / `alt` / `placeholder` / `label` 的字面量值若含中日韩文字即失败——这类属性会被屏幕阅读器和提示气泡直接念出来，写死中文就等于让英文界面冒出中文（`read-aloud` 的 `title="语速"` 是这一类的第一例，而写死的可访问名称此前已手工清过一轮、没有门禁所以又长回来） | 把值改成按 locale 取的 prop，调用方用 `locale === "en" ? … : …` 传入，并在用例里按语言断言该提示；英文写死的同一处它认不出来（判据是 CJK 字符），要靠按 locale 的断言补 |
