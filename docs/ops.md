...
| `npm run check:db-assertion-counts` | 现行文档（`docs/database-testing.md`、`docs/roadmap.md`）引用的 pgTAP 断言数等于 `supabase/tests/*.sql` 的 `select plan(N)`，含「`N+N+N 断言`」这类聚合写法。R16.83：声明的 2 篇文档少读到任何一篇都判失败（`docs/roadmap.md` 改名不会让引用自动变对），pgTAP 目录整个不见了也报错而不是让 `readdirSync` 抛一句读不懂的话。R16.268：`docs/roadmap.md` 只取 `## Q…` 那几节——`## R…` 之后是逐轮台账，那一行会将被改掉的旧数字原文引一遍当证据，整篇扫会把一句真话报成不符；节选之后只剩标题同样判失败，不按「没有不符」放行 | 改断言数后同步现行文档；`docs/progress.md` 与 roadmap 的 `## R…` 台账是追加式历史记录，不参与对账也不得回改 |
...
| `npm run check:dead-copy` | 字典死键巡检，两路口径加一张自检表（R16.21 + R16.78 + R16.272）。① `src/lib/i18n*.ts` 词条若在 `src/`（字典文件自身除外）、`e2e/`、`scripts/` 的非测试代码里没有任何引用点即失败。② 组件自己声明的字典接口，两种形状都收：名字带 `Dict` 的接口，以及 props 类型里内联的 `dict: {`（R16.272 之前只认前者，`AiQuiz` 的 `question`/`explain` 就是从内联写法里漏出去的）；两边都要求表里每个字段带 `string` 或 `string[]` 类型标注才判——对象字面量写的是值不是类型，所以组件本地那份 `const DICT = { zh: { heading: "…" } }` 不会被当成接口，混进回调或嵌套对象的整张跳过。某字段若从没挂在**这张字典在本文件绑定的那个变量**上被读一次，即失败——第一目对这类是瞎的：词条在页面有装配点（`error: t.chapter.aiSummaryError`）就算「被使用」，可接住它的组件一次都没读过它，改字典的人看着装配点真心以为改到了界面（R16.77 的 `error` 是第一例，R16.272 量出的四处是第二例）。变量名只从声明处推：props 标注 `dict: BannerDict`、内联块自己的键名、`const DICT: Record<…, BannerDict>`，再走一跳别名（`const dict = DICT[locale]`）；读过算三种形态——`dict.<字段>`、从 `dict` 解构、字段名以字符串常量出现在 `dict[…]` 的动态取法里（`replay-trainer.tsx` 的 `difficultyNew` 走的正是最后一种）。旧口径「字段名在声明块之外以任意形态出现过就算读过」已否掉：`ai-quiz.tsx` 里 `question` 出现 16 处，可读它的是 `q.question`（AI 返回的数据对象），那份界面文案一次都没渲染过；接收者也不能反过来从观测到的属性访问推，那样 `q` 会被升成字典变量、这一路当场又全绿。判定逐文件，别处读过不算；认得出是字典接口、却推不出它绑在谁身上的，报告第三张表逐条点名，判不动即失败——判不动不等于没问题。四路都从同一个 `scripts/dead-copy-budget.json` 取数：两项上限（死键 `budget`、未读字段 `dictFieldBudget`，当前均为 **0**）与两项下限（`minDictionaryKeys` / `minDictInterfaces`，钉在当前实测的词条数与接口数附近）。缺一键即判失败——拿到 `undefined` 的比较会永远通过；下限不是装饰：R16.81 里扫描器被接口内的一行块注释缴械、当场少扫一张接口，而下限悬在 10 这样的远低于实际的常数上，红绿一点没变，所以下限必须跟着实测量走（**少扫**东西要失败，多扫到不需要） | 直接删掉没人读的词条（连同假字典里的引用）；组件字段要么读掉，要么把接口字段和页面装配点一起删。确需保留的在 PR 里写明理由并上调预算，不得为了变绿而加 `// eslint-disable` 式的豁免 |
...
> 打印表面注记：`src/app/globals.css` 的 `@media print` 段自称「打印友好」，`e2e/print-surface.spec.ts` 就是这条承诺的检查——Chromium 里 `emulateMedia({ media: "print" })` 后，课文标题/正文/风险警示必须仍在纸上，站点顶栏、页脚、固定侧栏、☰ 目录浮钮与课文工具条必须不在。判据按「印出来的那一页该有什么」写，不按 CSS 的写法写：曾经的 `header, nav, footer` 一把梭把住在 hero `<header>` 里的 `<h1>` 一起吞了，而真正该隐藏的固定/粘滞外壳反而因为不是 `nav` 原样上纸。要新增一块打印时不该出现的外壳，给它加 `.no-print`（不能用 `aside { display: none }` 之类的标签级偷懒——兜底风险提示与「下一章」块也是 `<aside>`，那是内容宪法要求随课文一起印出来的）。
...
对**已部署的域名**跑 10 条只读断言，逐条打印后汇总；任一失败 exit 1。断言不是随手挑的，
但**也不是每一条都出过事故**：第三列写明来历——真出过事故的行，仓库里查得到落点；
从没出过的直写「无事故记录」，它在清单上是因为这一层的退化**在页面上看不出来**。第一列就是脚本打印的那个名字
（`scripts/prod-smoke.test.mjs` 逐字比对两边，改名或加一条而没同步这张表即红；
第三列空着、或写了来历却查不到落点，同样红在这里）：

| 断言 | 为什么在清单上 | 出处 |
|---|---|---|
| `GET /zh → 200 且含风险提示` | 中文首页：它挂了读起来像整站下线 | 无事故记录（这条要的是「整站下线级别的入口当场看得出来」） |
| `GET /en → 200 且含风险提示` | 英文首页：双语路由的另一侧只在这条上验证 | 无事故记录 |
| `GET /zh/knowledge/getting-started → 200 且含风险提示` | 课程页与课文页由知识库 markdown 生成，构建绿不代表这一层有内容 | 上游 README 里有非 pass 的篇目，靠站内兜底补上风险块：`docs/risk-warning-coverage.md` |
| `GET /zh/knowledge/getting-started/candlestick-basics → 200 且含风险提示` | 课文页是红线落地的最深处，兜底接线一旦被改掉这条就红 | 无事故记录——补兜底那一次仓库自己写的是「线上暂无暴露」（当时课文上游全 pass，见 `docs/risk-warning-coverage.md` 的 lesson 两行）；游客被判 500 那件真出过的事在 API 那一侧，是下面的 `#107` |
| `GET /sitemap.xml → 200 且是 urlset` | 抓取入口退化时页面本身看不出来 | 无事故记录（这个形状）；同一面上 R13.17 记下过两处产物缺陷（lastmod 失真、收录声明互相打架） |
| `GET /robots.txt → 200 且指向 sitemap` | 同上：robots 丢掉 `Sitemap:` 行，索引会慢慢掉 | 无事故记录；R13.17 在那一面记下的是 `Disallow` 与 `noindex` 打架，不是丢 `Sitemap:` 行 |
| `GET /zh/changelog → 含最新发布版本（部署跟上 main 的探针）` | 「合并 ≠ 上线」：Vercel 配额限流会让生产停在旧构建 | 0.7.2 当天合并打了 tag、生产停在 0.7.1 构建，见 `docs/progress.md` |
| `GET /share/streak/<合法载荷> → 200 且含风险提示（R14.7）` | R14.7：分享落地页曾经漏掉风险行 | R14.7 盘点逮到 `/share/*` 三类落地页完全没有承载面，已修复并加断言 |
| `GET /api/auth/session 匿名 → 200 {"user":null}（#107 游客判定回归）` | PR #107：游客被判 500 的回归 | PR #107（未登录访客请求 `/api/auth/session` 返回 500） |
| `POST /api/ai/chat 游客：护栏路径 200+X-Refused，模型路径要么答要么明确没测` | 拆成两段是因为一句「AI 502」分不清站内还是站外：护栏在调用上游**之前**就返回，它 200 且带 `X-Refused` 就证明函数活着、部署没落后、内容红线还在，此时模型路径 5xx 只可能是上游配置或出口。模型路径单独等 `AI_MODEL_TIMEOUT_MS`（90s）——生产实测上游不通时这一跳要挂约 68s 才回 502，用全站那 20s 只会得到「我们超时了」 | PR #107（游客 AI 问答 502，站内成因）；此后生产多次 502 的记录同样在 `docs/progress.md` |

`⏭️` 是第三种结论：**这一条本轮没测成**，既不算 ✅ 也不算 ❌，汇总里单独说一句。目前只有一个来源——
游客 AI 那条撞上 429（限流排在解析与护栏**之前**，所以护栏路径 429 时连内容红线都没被走过；模型路径单独
429 时红线验证过、但「模型路径不 5xx」这半句没测）。把它印成 ✅ 就是这份清单存在的理由要防的事。
...
而不是异常，所以重试不会把真实的站内回归刷成通过；非传输层异常一次定性。唯一的例外是游客 AI 那条的模型路径：
它自己等满 90s 后把「没返回」写成一条**结论**（护栏路径已经 200，重试只会再挂 90s 并白占访客配额），不参与重试。
