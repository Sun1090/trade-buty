...
- [ ] R16.21 死文案（字典里没人读的键）目前靠人工倒查，没有门禁 — `trendEmpty`（`src/lib/i18n-stats.ts`，「暂无近 7 天完成课程记录」）是本轮顺手删掉的第二条：全站只有 `stats-client.test.tsx` 里那份假字典引用它，组件从来不读。#203 一次删了 18 条同类的（其中还躺着一句已经不成立的「173 篇深度课程」）。这类键不会自己消失，而它们的危险不在于占字节，在于下一个人以为改了就改到了界面上。可以做一条保守门禁：对字典的每个键，要求在 `src/` 里（字典文件自身与测试假字典之外）至少出现一次同名标识符；先落地成报告式台账（与时钟卫生同构），误报逐条豁免后再升级为阻断
