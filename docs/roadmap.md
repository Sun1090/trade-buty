...
- [ ] Q5.4 [手动] 备份演练：Supabase 数据导出 + 仓库镜像确认 — 本地半边已完成：`npm run backup:drill` 全自动跑「迁移 → 全业务表（11 张）灌数据 → `pg_dump -Fc`(45,104 bytes，随语料增长) → 销毁源库 → 全新实例 `pg_restore` → 数据/schema/RLS/约束指纹比对 → 重跑 pgTAP(40+26+8 断言)」，脚本与边界见 docs/database-testing.md 第 4 节；`BLOCKED_EXTERNAL`：Supabase 云导出（auth/Storage/项目配置）、定时备份与仓库镜像确认需云控制台权限。
...
- [ ] R15.4 AI 边界的 C 级盘点结论（**已核实、未修**，按收益排队，不擅自动手）：①只有 `/api/error-reports` 对请求体做了字节上限（`readBoundedBody`），其余 AI 路由直接 `req.json()`，目前靠平台体积上限与「限流先于解析」兜着；②`ai/quiz` 的兜底分支返回 `fixedQuiz.questions` 时不再按 `locale` / `difficulty` 过滤，英文用户可能拿到中文卷，且该分支跳过了变体模式用的 `CHAPTER_SLUG_RE`（只因为 `getChapterTitle` 走白名单才没变成注入面）；③限流的 IP 维度建立在可伪造的 `x-forwarded-for` 上（`src/lib/ai/rate-limit.ts:9-11` 自己写明了），并且 `BoundedMap` 满 10_000 键会挤掉仍在窗口内的真实计数，等于攻击者可用大量新 IP 让老配额清零；④`/api/auth/session` 与 `/api/auth/signout` 在站内没有任何调用方（只有文档与 `ops:smoke-prod` 用 session 做回归探针），也无限流，而每一次都要跑一趟 Auth API
...
- [x] R16.4 复习页与统计页的「今日到期」不是同一个数：`review-client` 的头部计数与置顶排序读原始 `srsDue`，行内徽章、`stats-client`、`streak-recovery-card` 读 `effectiveSrs()`（R5.6 的回填）。无 `srs_due` 的条目（R5 之前的旧数据、云端该列为空的行）于是同时被算成「今日到期」并显示「1 天后」——同一屏两个数字打架，且断签提醒卡的口径与复习页也不同。统一到 `effectiveSrs()`；`wrongbook-efficiency` 的 `overdue`（旧数据不标红）另有已固化断言，属另一个入口，暂不并入
