...
- [x] R7.6 前端错误上报分级（fatal / recoverable / silent 三档；`reportRouteError` 已接入路由级 `error.tsx`，单测覆盖三档、digest 元数据、非 Error 输入与上报自身失败。自定义根级 `global-error` 经实测会给全部 454 条路由各加 13.4KB gzip JS 并击穿 R13.15 预算，故保留 Next 内置兜底页）
