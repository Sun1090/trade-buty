...
这 10 条断言不是随手挑的，但**也不是每一条都出过事故**：`docs/ops.md`「生产冒烟」那张表第三列逐条写明
来历——真出过事故的给得出仓库里的落点，没出过的直写「无事故记录」，它在清单上是因为那一层的退化
**在页面上看不出来**。下面摘几条最容易读错的：

- `GET /api/auth/session` 匿名访问返回 `200 {"user":null}`，不是 500（游客判定回归，PR #107）。
- 未登录状态下 AI 问答可用，不返回 5xx。撞上 429 时脚本印 ⏭️「本轮没测成」而**不是** ✅：限流排在
  解析与护栏之前，那一轮连内容红线都没走过（模型路径单独 429 时红线已验证，但「不 5xx」这半句仍没测）。
...
- 对本地构建冒烟前，先确认 3111 上没有**上一次留下来的 `next start`**：驻留的旧服务会让整轮结果
  失真（旧构建的缺陷算成本次的红，本次的缺陷藏成绿）。现在脚本自己会查——本地目标的响应 HTML 里
  必须含当前 `.next/BUILD_ID`，否则直接 exit 1 并说明原因，不会把 10 条断言跑完再让你猜。
