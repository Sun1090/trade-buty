...
机械可查的那一半每次 CI 都重跑：`scripts/growth-event-privacy.mjs` 的禁区有 **6 类出口**——网络（`fetch` / `sendBeacon` / `XMLHttpRequest` 三条各算一类）、持久化存储、cookie、剪贴板；它还要求出口只有一个 `console.info`、事件必须过白名单重建，并核对下面目录里的每个事件名都还在文档里。这一半过期即红。需要人判断的那一半（「算不算远端收集」）跟着出口走：出口一变就要重审。所以这里不写「最后更新」的日期。
