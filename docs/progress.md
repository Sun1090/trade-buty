...
- 我自己的错（本轮五处）：**①判据被同一格里的另一句话白满足**——「405 是谁答的」我写成 `toContain("路由层")`，而下一句「走不到路由层那一格」也含这三个字，于是把归属那句改成「的缓存层挡下」判据照样绿；是探针 M6 报 BAD 才暴露的，改成匹配整句形状（`/其余方法由 Next\s*的路由层挡下/`）之后 M6 才红。这是 R16.256 那两条、R16.253 §8 那条的**第三次同形**：`toContain` 的一元参数如果是「一个词」而不是「一句带结构的话」，它就还没在测那件事。**②修台账时只修了我看到的那一处**——上一提交（`7eda295`）我在「我自己的错」条目里承认了「两条分支都没有用例覆盖」是假缺口，可同一份 `docs/progress.md` 里那条「下一项」仍然写着这句话；本轮写台账时回读才发现，也就是说**我先把假说法记进了一处、又把对它的更正写在另一处，两句在同一个文件里并存了一小时**。**③我自己那条更正的第一版又是错的**：改成「媒体类型那一半有：`src/app/api/error-reporting` 的 `route.test.ts:132`」——目录名是我凭空拼的，真路径是 `src/app/api/error-reports/route.test.ts:132`。读自己的 diff 才发现（`git diff` 就在那儿，是我不看）。**④前三版探针根本没造出要测的条件**：R16.266 那句「断网点应用内链接会怎样」，我第一版断网点链接、第二版掐 `?_rsc=`、第三版改点页脚，三次都渲染出真页面——因为站内链接同时都在顶部视口导航里、payload 早被预取，点击压根不上网。要是我就把这三次读数记成「实测：断网时应用内导航正常」，那是一条比原文档更错的结论。救回来的是第二版里一条我没解释的副作用：掐断 RSC 之后**多出一条 `document` 请求**，那才是「回退成整页导航」的入口，第四版照这个设计才量到离线壳。**⑤脚本改台账只数了行数、没读内容**：R16.266 那一行我用 node 整行替换，验了「789 → 789 行」就提交，结果行里留着一个坏掉的内联代码块——「`global-error.tsx)`」（反引号没闭合、多了个全角括号），`check:docs` 那类门禁不看这个。已在下一版修掉，但它进了 `d641a9e`。
- 验证：`caching-claims` 19 条 + `error-report-claims` 12 条 + `doc-anchor-claims` 56 条 + `route.test.ts` 15 条绿；`.gate-logs/probe-r33.mjs` **7 条 BAD=0**（6 条改坏真相全红 + 1 条只动 §4 的反向对照绿），`.gate-logs/probe-r33b.mjs` 首跑 **11 条 2 BAD**（①②两处即上文的 M6 与我对 M1 预期文案的误判），修完重跑 **11 条 BAD=0**，两批跑完 `git status` 只剩本轮该改的文件。**全量链** `.gate-logs/chain-r33.sh`（沿用 R16.260 那套从 `ci.yml` 派生步骤、按 CI 顺序、开头删两份 gitignore 产物的写法）：**50 步 1 红**，红的是 `check:report-freshness`，且红得对——它比对的是「工作区 vs HEAD」，而我当时还没提交重算出来的两份台账；`c345664` 之后单跑该门禁读「18 份 · 漂移 0 · 未提交 0」exit 0。其余 49 步全绿：`lint` exit 0（新判据文件里一个未使用的绑定被 `--max-warnings=0` 挡下，已删）、`test:coverage` 在**没有产物**的工作区里 **336 文件 / 3493 条**全绿（Lines 97.1%）、`build` 出 **474** 页并把产物写回、`e2e` **164 passed (2.0m)**、`git status` 除本轮台账外干净。
- 阻塞 / 风险：无新增阻塞。`docs/caching.md` §5 里「那条路径由 `src/app/error.tsx` 承接」这一句本轮**没有实测**（它是上一轮就在的旧说法，本轮只重写了它前后两句）——R16.266 登记为待量：应用内点链接断网时到底弹的是根错误边界还是别的，需要一次真浏览器交互实验，不能靠读代码定。R16.235（首屏 59.3KB Supabase SDK）仍等能端到端验证登录态；R16.98 与 PR #178 仍等你拍板。
