...
- 下一项：PR #316 等 `ci` + `db-tests` + CodeQL 绿了 `gh pr merge 316 --rebase --admin`，本地 `main` 快进；然后第二十四轮做我已读码核实的三条：`i18n.ts:338/749` 邮件订阅那句「功能开发中 / under construction」（同一张卡标题写「（占位）」，而 roadmap 里 R8.8 早已 `[x]`、没有任何在建的邮件后端条目）、`i18n.ts:549` 断网那句 "will retry automatically when you are back online" 与实际的重连逻辑对不对得上、`i18n.ts:718` 回放空态那句 "your training log will appear here" 的真伪。仍等用户拍板：R16.159 / R16.164 / R16.174 / R16.205 / R16.214 / R16.215，外加生产 AI 502 需要 Vercel 控制台权限。
