...
- 里程碑 / 版本：「说法 vs 事实」第三十五轮，无发布（当前 v0.7.18）。分支 `fix-database-testing-claims-r35`（自本地 `main` `1313427`）→ 本记录走 PR 合并：`0525eac`（pgTAP 两处补齐）、`8e2bcd0`（说明书逐格改写成工件自己说的数 + 新判据）、`bca14ee`（roadmap 两行按形态重写 + 判据换腿）、`0e54048`（R16.268 登记）、`cc7813f`（节选范围收成一个主人）、`73c9f18`（R16.269 登记）、`1178c3d`（R16.270：备份字节数改口 + 两条判据）、本轮记录。
...
- 完成内容 **R16.270**（PR 开出之后在同一轮内补）：roadmap Q5.4 那格写「`pg_dump -Fc`(45,104 bytes，随语料增长)」——括号的来源是某一次演练打印的读数。本地重跑读到 `45104` 让我以为「验过了」，**CI 的同一个 `db-tests` 作业读到 `45105`**。「随语料增长」这个限定只承认数会随内容变，没承认同一份内容在两台机器上也不是同一个数。改成只说判据与出处：字节数每次由脚本自己打印、两处读数各带归属、脚本对它唯一的真判据是 1024 字节下限；判据两条 + 探针 5 腿 BAD=0（`.gate-logs/probe-r35-bytes.mjs`，含「只动台账节 → 绿」的反向对照）。
