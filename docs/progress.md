...
- 两条**核查后不成立**的候选，记下来免得下次重复怀疑：①`.next` 那一系产物门禁（`check:risk-warning` / `check:search-index` / `check:structured-data` / `check:seo-surface`）会不会又犯 R16.83 那个「扫到 0 个照样绿」——读下来不是缺口：前者以知识库为真值逐页回读产物，后两者拿 sitemap 与产物**双向对账**，产物空了或只建出一半都会当场红；只有「产物齐全但来自上一次构建」这一种形状没被覆盖，而它在 CI 里不存在、本地由 R16.37 那道 BUILD_ID 检查兜着。②两个互不知情的只读探查代理分别扫「书签 / 复习 / 更新日志 / PWA 安装 / 隐私导出」与「AI 聊天与端点 / 课文页阅读进度与朗读 / 测验与章节测」，**各 0 条成立发现**（前者读了 `bookmark-store` 的导入合并、SRS 四状态路由与当日配额、`install-prompt` 的 never-ask 与 iOS 分支、导出 v2 的字段清单；后者核了游客额度两处计数同源、答案缓存键与 500 字符切片、`speech-wrapper` 的作用域、`quizScorePct/Count` 封顶与分享载荷的解码拒绝路径），并确认「账号切换后旧响应写脏状态」这一类 `isCurrent` 问题在本仓库不存在。**空手而归也是数据**：这两片表面的账可以结掉，下一轮该换没扫过的地方。
...
- 更新时间：2026-09-24 17:15（Asia/Shanghai）。
