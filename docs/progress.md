...
- **订正我上一条 bullet 里的一句话**：我写「两个只读探查代理各 0 条成立发现」，**这话对其中一个是错的**。扫「AI 聊天与端点 / 课文页 / 测验」的那一轮确实空手而归（并确认「账号切换后旧响应写脏状态」这一类 `isCurrent` 问题在本仓库不存在）；但扫「书签 / 复习 / 更新日志 / PWA / 隐私导出」的那一轮交回 4 条，我逐条读源码复核后**3 条成立、1 条要拍板**：①复习页 intro 承诺「看懂解析后点『已掌握』移出」，而默认（SRS 开）根本没有那个按钮——它渲染的是「掌握了」，走的是 `applySrsResult` 的 5 档间隔（`srs.ts:22` 的 `[1,3,7,14,30]`），只有最后一档答对才真的出库；`resolved`「已掌握，移出错题本」只在 SRS 关闭时出现。②AI 变体题的「已举报」在 `fetch` 之前就被 `setReported` 写死（`ai-quiz.tsx:101-111`，响应不读、错误 `.catch(() => {})` 吞掉），按钮随即 `if (reported[idx]) return` 再点不动——离线/401/429/500 都会打印「已举报」，而且不给重试；`ai-quiz.test.tsx:233` 那条「举报网络失败不影响已举报状态」是在替这个 bug 作证（与 R16.33 同形状）。③`privacy-export.ts:8` 的文件头说同步摘要带「上次 hydrate 时间…登录态」，而 `SyncSummary`（`:57-70`、`buildSyncSummary` `:158-169`）实际导出的是队列长度/nextId、**上次访问**（`lastVisitAt`，不是 hydrate）、引导状态与邀请参数，`tb-last-cloud-sync` 从未被读。④`src/app/manifest.ts` 把安装后的 `start_url` 钉在 `/zh`、`lang: "zh-CN"`，而英文页的安装引导文案是英文、`i18n.ts:3` 的 `DEFAULT_LOCALE` 是 `en`——英文访客装上来的那个「应用」是中文站。①②③ 是代码与文案对不上，归下一批；④ 要定「装哪个语言的站」，归 roadmap 待拍板。
...
- 更新时间：2026-09-24 17:20（Asia/Shanghai）。
