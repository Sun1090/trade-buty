...
## 2026-09-26 · 第三十三轮（R16.264–R16.265）：离线壳的前提与 405 的归属，都先量再写

- 里程碑 / 版本：「说法 vs 事实」第三十三轮，无发布（当前 v0.7.18）。分支 `fix-doc-claims-round33`（自 `290e76f`），三条提交：`96deba3`（§5 改写 + 判据）、`d9947a7`（error-reporting 拆格 + 新判据）、`c345664`（两份计数台账重算 + 一个未用绑定）。
...
- 门禁：`scripts/caching-claims.test.mjs` 新增 §5 那一组（16 → **19** 条）；新增 `scripts/error-report-claims.test.mjs`（**12** 条：方法导出唯一、源码里没有 405 分支、限流判在媒体类型之前、415 的判据写法、两码归属、四处被引用的行号、body / 限流两把尺子的数值）。
