...
- 下一项：#290 等 `ci` 与 `db-tests` 绿后 rebase 合并；#289 已把「两轮探查各 0 条发现」的错账订正到底（`e1f2871`），它先绿就先合，本分支随后 rebase 到新的 main（`docs/roadmap.md`、`docs/progress.md` 是尾部追加，冲突按「两段都留」处理）。可执行的下一条已经排着：把 R16.78 那条线补完（计数型门禁的下限改成「相对上一次入库快照不得下降」），以及第三只眼还没扫过的表面——课程正文渲染管线与 `/path` 的进度条。
