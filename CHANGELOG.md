...
- 两处只在满负载跑全量时才红的用例修好了（回放与登录水合那两条现在等的是真正会发生的那一次调用，而不是等屏幕上出现某个元素）；`check:bundle` 的容差按构建机之间的实测噪声重定，任何往字典加一句话的 PR 不再随机把 CI 弄红
...
- Two cases that only went red under a fully loaded run were fixed (the replay and login hydration tests now wait for the call that actually happens, not for an element to appear on screen); the check:bundle tolerance was re-derived from measured build-to-build noise, so a PR that adds a single dictionary sentence no longer randomly reddens CI
