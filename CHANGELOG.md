...
- 换账号不再丢掉答题难度：分语言的两颗偏好（`tb-quiz-difficulty:zh` / `tb-quiz-difficulty:en`）以前会被当成「上一个账号的分章成绩」一起清掉，换个账号回来难度静默回到默认。归属判断现在认前缀，派生键不再算成绩——这条用户可感的数据丢失就是本轮判级为 patch 的理由
- 关于页的邮件订阅那块不再写「功能开发中」——站内既没有邮件服务，也没有它的排期；那段说明也不再指挥用户去点此刻屏幕上并不存在的两颗按钮
- 三处界面承诺收回到它真做得到的范围：安装提示的拒绝按钮改说它真正做的事（永久关闭），不再写「暂不」；活动热力图空态那颗按钮不再替一节它并不指向的课作保；`/calendar` 页脚不再替一条没有排期的路线图作保
- 生产冒烟不再把「这一轮没测成」报成通过：游客 AI 撞上 429 时改印 ⏭ 并说明那一轮连内容红线都没走过；上游慢也不再被那条通用超时报成「请求异常」
- 12 份文档与审计报告里对用户或维护者可见的句子逐条回代码量过：留存指标表、增长文案政策与事件目录、架构文档（里面有一个从来没有过的存储键）、缓存那份与环境变量那份；`docs/architecture.md` 里那句伪造的宪法引文已撤下。这一轮共加 8 份现读判据把这些句子钉住
- 四条偶发红的断言修好了：回放那两条现在等的是 `useEffect` 里那次入库真的落地，登录水合那两条等的是动态 `import()` 之后才被调到的那个函数——以前它们只等屏幕上出现某段文字，机器不忙时看不出问题。`check:bundle` 的判定也不再取决于「谁构建的」：同一份代码在 CI 与本地实测差 2.2KB，那条预算从 400KB 放宽到 404KB（这一档差的成因未查明，已登记待办），任何往字典加一句话的 PR 不再随机把 CI 弄红

### English

- Switching accounts no longer wipes your quiz difficulty: the two per-language preferences (`tb-quiz-difficulty:zh` / `tb-quiz-difficulty:en`) used to be swept away as if they were the previous account's per-chapter scores, so the level silently reverted to its default after a switch. Ownership now matches on prefix and derived keys no longer count as scores — that user-visible data loss is why this release is a patch
- The newsletter block on the about page no longer says the feature is in development — there is no mail service in the product and no schedule for one — and it no longer tells you to press two buttons that are not on the screen
- Three interface promises were pulled back to what the code actually does: the install prompt's dismiss button now names what it really does (close it permanently) instead of not now; the activity calendar's empty state no longer vouches for a lesson it does not link to; the /calendar footer no longer vouches for a roadmap item that was never scheduled
- The production smoke no longer reports a round it never got to test as a pass: a guest AI request that hits 429 now prints a skip marker with a note that the content red lines were never exercised that round, and a slow upstream is no longer reported as a request error by a generic timeout
- Every user- or maintainer-facing sentence in 12 docs and audit files was re-measured against the code: the retention-metrics table, the growth copy policy and event catalog, the architecture doc (which named a storage key that never existed), the caching doc and the environment-variable doc; the fabricated constitution quotation in docs/architecture.md has been withdrawn. 8 new gates read the code live and pin those sentences
- Four intermittently-red assertions were fixed: the two replay ones now wait for the save that happens inside `useEffect` to actually land, and the two login-hydration ones wait for the call that only happens after a dynamic `import()` — previously they only waited for some text to appear on screen, so an idle machine hid the problem. The check:bundle verdict no longer depends on which machine built it: the same code measured 2.2 KB apart between CI and local, so that budget was widened from 400 KB to 404 KB (the cause of the gap is unestablished and recorded as an open item), and a PR that adds a single dictionary sentence no longer randomly reddens CI
...
- ☁「已云端存档」等到离线写队列真的清空才出现：过去只要某一次写成功就亮徽标，队列里还压着没送出去的分片时界面已经在说「都上云了」；现在徽标的时态与实际排空一致，没排空时说的是「还要传」
- 回放的「跳到末尾」真的把整段填进图表：以前它只把播放位置挪到最后一根，图表上仍停在你看到的那一段；⏭ 现在按末尾重画整段，趋势块也改口——历史不够一轮时直说缺多少根，分母不再写成负数
- 行情卡片那个百分比说清楚了是 24 小时的涨跌，不是「这一跳」的变化；箭头旁边一句话把窗口口径交代完，不会再让人以为刷新一次数字就该变
- 自定义模式的「新一轮」往前另取一段历史：以前按下去只是把同一批请求重发一遍，屏幕上还是那一轮；现在会在你填的截止日期之前重新取样，日期输入框跟着走，也不会反过来改写你手动填过的截止日
- 图表交易对输入的三种失败各得到一句真话：形状不对当场在输入框下面说清规则、改对之后自动消失；币安现货没有这个标的时说「没有这个交易对」而不是「API 可能不可达」（浏览器读不到跨源的 400，改由一次 ping 判定主机活着但没有这个标的），并且不再给一个点了必然得到同一句的「重试」
- 英文树的内容下限变成常驻门禁：en 目录里每个文件都必须去掉头信息之后非空、中日韩字占比不超过 2%、正文不少于同名中文的 30%。当场实测 209 个文件与中文 1:1 对齐、最高占比 0.52%、最短的一篇也有中文的 1.70 倍——界面上那句「英文与中文覆盖同样的 27 个篇章」现在有内容层面的支撑，不再只是目录数

### English

- The ☁ "Saved to cloud" badge now waits until the offline write queue has actually drained: it used to light up after a single successful write while chunks were still queued, so the interface said everything was upstream while it was not. The tense matches reality now, and a partially queued write says "still to send".
- "Jump to end" in replay really redraws the whole segment into the chart: previously it only moved the playhead to the last candle and left the chart showing the slice you were looking at. ⏭ now repaints the series up to the end, and the trend block stops asking for a round you already did - when the window is too short it says how many candles are missing instead of printing a negative denominator.
- The percentage on the market card now says out loud that it is a rolling 24-hour change, not this tick, so a refresh no longer looks like it failed to update the number.
- "New round" in custom mode really takes a different slice of history: pressing it used to resend the identical request and leave you on the same round. It now samples a fresh window before the end date you filled in and moves the date input with it - without overwriting a deadline you typed yourself.
- The chart's symbol input answers each of its three failures truthfully: a malformed pair is called out under the box and the note clears once you fix it; a pair Binance spot does not list is reported as "no such trading pair" rather than "Binance API may be unreachable" (a browser cannot read that cross-origin 400, so a single ping decides whether the host is alive but the symbol missing), and the failure state no longer offers a "Retry" that is guaranteed to return the same sentence.
- The English tree got a standing content gate: every file under en/ must have a non-empty body after its frontmatter, keep CJK characters under 2%, and be at least 30% as long as its Chinese counterpart. Measured on the spot: 209 files aligned 1:1 with Chinese, the highest CJK share is 0.52%, and the shortest file is still 1.70x its Chinese source - so "English and Chinese cover the same 27 chapters" now has content behind it, not just directory counts.
