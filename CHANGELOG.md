...
- AI 对话复制回答失败时此前完全静默（`catch` 吞掉异常、且没有兜底），在微信内置浏览器等没有 `navigator.clipboard` 的环境按钮毫无反应；现在统一走 `copyText`，复制失败可见「复制失败」文案
- 新增统一的剪贴板助手 `src/lib/clipboard.ts`（异步 Clipboard API + `execCommand` 兜底，返回真实结果）并替换四处重复实现；补齐搜索快捷键、代码复制与各复制组件用例，全局覆盖率提升至 91.87% / 86.13% / 91.44% / 94.36%
...
- Fixed silently-failing AI answer copy: the old `catch` swallowed errors and had no fallback, so the button did nothing in WeChat's in-app browser where `navigator.clipboard` is absent; copying now goes through `copyText` and surfaces a visible “Copy failed” message
- Added a shared clipboard helper `src/lib/clipboard.ts` (async Clipboard API with an `execCommand` fallback that returns the real result) and replaced four duplicated implementations; added coverage for the search hotkey, code copy, and the copy components, lifting global coverage to 91.87% statements / 86.13% branches / 91.44% functions / 94.36% lines
