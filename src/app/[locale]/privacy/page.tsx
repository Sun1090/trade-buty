  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-5 py-10 sm:py-14 space-y-6 text-sm text-muted leading-relaxed">
      ...
      <section className="space-y-3">
        ...
        <p>{locale === "en"
          ? "The AI tutor works without an account, which makes it the only thing that sends what you type off your device before you log in: your question and the chapter you are reading are passed through our own server to the AI provider named under Third-Party Services. Conversations started while you are logged out are never written to our database; conversations started once you are logged in are written to our database: each finished turn saves your question and the full answer text under your account identifier, so the AI page can restore your recent history on another device. Two clicks are stored in the database, though - rating one of the answer helpful or unhelpful saves that rating together with your question and the full answer text, and opening a citation in an answer saves the chapter or lesson it points to together with your question. While you are logged out these two kinds of rows carry no account, no email address and no device identifier; once you are logged in they are saved with your account identifier attached, so a reviewer can tell whose rating it was. Neither kind ever carries an email address or a device identifier, and they exist only so a human can review answer quality - what happens to them when you delete your account is spelled out under Data retention & cleanup."
          : "AI 助学不需要账户就能用，未登录时它是唯一会把「你输入的内容」送出设备的功能：你提问时，问题和你正在阅读的篇章会经本站服务器转发给「第三方服务」一节点名的 AI 服务商。未登录时开始的对话不会写入我们的数据库；登录后的对话则会被写入我们的数据库：每完成一轮问答，你的提问与那条回答全文都会带上你的账户标识存进去，用于换设备时恢复最近对话。另有两种点击会写入数据库——给某条回答点「有用/没用」时，这次评分连同你的问题和该条回答全文一起写入；点开回答里的某条引用时，写入的是该引用指向的篇章/小节以及你的问题。未登录时这两类行是匿名的：不带账户、不带邮箱、不带设备标识；登录后它们会带上你的账户标识，让复核的人知道这条评分出自哪个账户。两类行都不会带上邮箱地址或设备标识，也只用于人工复核答案质量——删除账户时对它们的处理写在下方「数据保留与清理」。"}</p>
      ...
      <section className="space-y-3">
        ...
        <p>{locale === "en"
          ? "Local data lives only in your browser's storage: learning records, the mistake log and settings sit in localStorage, while a few interface flags such as \"should this tip still show\" sit in sessionStorage and disappear when the tab closes (they are not part of your account data). Nothing is cleared from your side until you clear site data. Before you log in our servers keep no personal profile: crash diagnostics go to short-lived logs and never reach the database, and while you are logged out the two kinds of AI feedback rows named above hold no account, email address or device identifier. Built-in trims keep the local footprint small: the study-time ledger keeps the 90 calendar days ending at the day you last studied, and market replay history keeps the latest 100 rounds."
          : "本机数据只存活在你浏览器的本地存储里：学习记录、错题本和设置在 localStorage，另有几个「这条提示还要不要显示」之类的界面状态写在 sessionStorage——关掉标签页就没有，也不属于账户数据。在你清除站点数据之前，这些都不会被动。未登录时服务器不保存任何个人资料：崩溃诊断只写入短期日志、从不写入数据库，上方那两类 AI 反馈行在未登录时也不含账户、邮箱或设备标识。为控制本机体积，学习时长台账以你最后一次学习的那一天为终点向前保留 90 个日历日（不是「从今天往前 90 天」：几个月没打开，留下的就是几个月前那一段），行情回放历史仅保留最近 100 轮。"}</p>
        <p>{locale === "en"
