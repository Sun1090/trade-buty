    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-5 py-10 sm:py-14 space-y-6 text-sm text-muted leading-relaxed">
      ...
      <section className="space-y-3">
        ...
        <p>{locale === "en"
          ...
          : "本机数据只存活在你浏览器的本地存储里：学习记录、错题本和设置在 localStorage，另有几个「这条提示还要不要显示」之类的界面状态写在 sessionStorage——关掉标签页就没有，也不属于账户数据。在你清除站点数据之前，这些都不会被动。未登录时服务器不保存任何个人资料：崩溃诊断只写入短期日志、从不写入数据库，上方那两类 AI 反馈行在未登录时也不含账户、邮箱或设备标识。为控制本机体积，学习时长台账仅保留最近 90 天，行情回放历史仅保留最近 100 轮。"}</p>
        <p>{locale === "en"
