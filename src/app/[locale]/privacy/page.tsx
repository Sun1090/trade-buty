  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-5 py-10 sm:py-14 space-y-6 text-sm text-muted leading-relaxed">
      ...
      <section className="space-y-3">
        ...
        <p>{locale === "en"
          ? "Local data lives only in your browser's localStorage and stays until you clear site data. Before you log in our servers keep no personal profile: crash diagnostics go to short-lived logs and never reach the database, and the anonymous AI feedback rows named above hold no account, email address or device identifier. Built-in trims keep the local footprint small: the study-time ledger keeps the most recent 90 days and market replay history keeps the latest 100 rounds."
          : "本机数据只存活于浏览器 localStorage，直到你清除站点数据为止。未登录时服务器不保存任何个人资料：崩溃诊断只写入短期日志、从不写入数据库，上方那两类匿名 AI 反馈行也不含账户、邮箱或设备标识。为控制本机体积，学习时长台账仅保留最近 90 天，行情回放历史仅保留最近 100 轮。"}</p>
        <p>{locale === "en"
          ? "Cloud data (only after you log in) is kept while your account exists so learning progress survives across devices. Pending offline writes are retried until they sync or the account is removed. To delete everything, sign in and use the account menu → Delete account: it removes every cloud row owned by your account - progress, mistake log, quiz scores, replay history, settings, and the ratings you gave AI answers - and clears local data on that device. One exception: citation clicks are detached from your account at that moment and kept as anonymous rows, holding only the chapter or lesson you opened and the question asked, which can no longer be tied back to you."
          : "云端数据（仅在登录后存在）在你的账户存续期间保留，用于多设备同步；离线写入队列会持续重试，直到同步成功或账户被删除。如需彻底清理：登录后在账户菜单选择「删除账户」，会删除该账户名下的全部云端数据（进度、错题、测验成绩、回放历史、设置，以及你给 AI 回答打过的评分）并清除本机数据。唯一例外：你点开过的 AI 引用记录会在那一刻摘掉账户标识、以匿名行保留，其中只有你打开的篇章/小节与当时的提问文本，且不再能对应回你。"}</p>
      </section>
      <section className="space-y-3">
