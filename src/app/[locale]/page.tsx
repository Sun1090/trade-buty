...
export default async function Home({ params }: PageProps<"/[locale]">) {
  ...
  return <div className="bg-[var(--background)]">
    <section className="border-b border-[var(--border)] bg-[radial-gradient(circle_at_80%_0%,rgba(52,211,153,.14),transparent_42%)]">
      <div className="mx-auto max-w-6xl px-5 py-10 sm:py-16">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
          <TodayPick chapters={learningChapters} locale={locale} label={locale === "zh" ? "你的下一步" : "Your next step"} hint={locale === "zh" ? "继续课程" : "Continue course"} done={locale === "zh" ? "全部课程都读完了，复习或挑战练习吧" : "Every lesson read. Review or practise next."} />
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2"><GlobalReadStat chapters={chapters.map((chapter) => ({ slug: chapter.slug, docCount: chapter.docCount }))} textTpl={t.home.readTpl} keepGoing={t.home.readKeepGoing} syncedLabel={t.home.syncedLabel} /><DailyTip locale={locale} nextTipLabel={t.home.nextTipLabel} /></div>
      </div>
    </section>
    <div className="mx-auto max-w-6xl px-5 py-14 sm:py-20">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{chapters.slice(0, 6).map((chapter, index) => <Link key={index} href={knowledgeHref(locale, chapter.slug)} className="group rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 transition hover:-translate-y-1 hover:border-accent/50 hover:bg-[var(--surface-hover)]"><div className="flex items-center justify-between"><span className="font-mono text-xs text-accent">{String(index + 1).padStart(2, "0")}</span><span className="text-xs text-faint">{chapter.docCount} {locale === "zh" ? "节课" : "lessons"}</span></div><h3 className="mt-5 font-semibold group-hover:text-accent">{chapter.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-relaxed text-muted">{chapter.tagline}</p><div className="mt-5 text-xs font-medium text-accent">{locale === "zh" ? "进入课程" : "Open course"} <span className="transition group-hover:translate-x-1">→</span></div></Link>)}</div>
