...
export default async function Home({ params }: PageProps<"/[locale]">) {
  ...
  return <div className="bg-[var(--background)]">
    ...
    <div className="mx-auto max-w-6xl px-5 py-14 sm:py-20">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{chapters.slice(0, 6).map((chapter) => <Link key={chapter.slug} href={knowledgeHref(locale, chapter.slug)} className="group rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 transition hover:-translate-y-1 hover:border-accent/50 hover:bg-[var(--surface-hover)]"><p className="text-xs text-faint">{chapter.docCount} {locale === "zh" ? "节课" : "lessons"}</p><h3 className="mt-3 font-semibold group-hover:text-accent">{chapter.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-relaxed text-muted">{chapter.tagline}</p><div className="mt-5 text-xs font-medium text-accent">{locale === "zh" ? "进入课程" : "Open course"} <span className="transition group-hover:translate-x-1">→</span></div></Link>)}</div>
