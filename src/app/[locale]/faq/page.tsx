...
const FAQ_ZH = [
  ...
  { q: "图表是实时行情吗？", a: `是的，K 线数据来自 Binance 公开 API；浏览器报告数据节省或慢速网络时，实时更新会暂停，图表上会写明。图表提供 ${symbolListLabel(CHART_QUICK_SYMBOLS)} 共 ${CHART_QUICK_SYMBOLS.length} 个快捷币对，也可以直接输入任意以 USDT 计价的币安现货交易对。${REPLAY_SCOPE_ZH}` },
...
export default async function FaqPage({ params }: PageProps<"/[locale]/faq">) {
  ...
  const chosen = en
    ? [
        ...
        { q: "Is the chart real-time?", a: `Yes — candlestick data comes from Binance's public API; when your browser reports data-saver or a slow connection the updates pause and the chart says so. The chart has ${CHART_QUICK_SYMBOLS.length} quick pairs (${symbolListLabel(CHART_QUICK_SYMBOLS)}) and accepts any Binance spot pair quoted in USDT that you type in. ${REPLAY_SCOPE_EN}` },
