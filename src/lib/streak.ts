 ...
 * 调用方：`progress.ts` 的 markRead、`wrongbook.ts` 的 applySrsResult / recordWrong /
 * resolveWrong、`progress-helpers.ts`。随堂测不直接调它——答对走 resolveWrong、答错走
 * recordWrong，复习评「已掌握/还没」走 applySrsResult，三条都已经记过活动，
 * 所以做完一套题必然算一天。
 */
