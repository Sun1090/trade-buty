 ...
 * （扫描用例逐条核过），因此不做代码区间保护。
 *
 * 最后一步把所有残留的尖括号整体删掉，而不是只删「看起来像完整标签」的片段：一条
 * `</?[a-zA-Z][^>]*>` 挡不住没闭合的 `<script`，也挡不住 `<scr<scriptipt>` 这种删完一段
 * 又拼回一个标签的写法。这批字符串只流向 React 文本节点与 `<meta content>`（两处都转义），
 * 但半消毒的值比没消毒更危险——它会让人以为可以往别处送。实测当前内容里首段与摘要
 * 没有一个含尖括号（zh/en 两棵树下 0 命中），所以这一步不改变任何一句真实文案。
 */
