...
describe("plainText（R16.55：导语与摘要里的 markdown 语法不该原样上屏）", () => {
  ...
  it("没闭合的标签留不下尖括号：成对区间整段去掉，落单的括号只删自己", () => {
    // `<script` 没有右尖括号，配对失败，只删掉那个 `<`，后面的文字照常留着
    expect(plainText("停在 <script 结尾")).toBe("停在 script 结尾");
    // 嵌套写法：从第一个 `<` 找到第一个 `>`，整段吃掉，拼不出第二个元素
    expect(plainText("拼不回来 <scr<scriptipt>")).toBe("拼不回来");
    // 落单的右括号同样不留：它没有配对的左括号，整段区间无从谈起
    expect(plainText("收益 > 3% 的场合")).toBe("收益 3% 的场合");
  });
