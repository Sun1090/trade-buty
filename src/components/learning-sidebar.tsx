...
export function LearningSidebar({ locale, labels }: { locale: string; labels: { learn: string; practice: string; review: string; stats: string; bookmarks: string; search: string; ai: string } }) {
  ...
  return <aside className="no-print fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-[var(--border)] bg-[var(--background)]/95 px-4 pb-6 pt-24 backdrop-blur-xl lg:block">
    <nav className="space-y-7" aria-label={locale === "zh" ? "主导航" : "Main navigation"}>
      {groups.map((group) => <div key={group.title}><p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-faint">{group.title}</p><div className="space-y-1">{group.items.map((item) => { const active = pathname === `${prefix}${item.href}` || (item.href !== "/" && pathname.startsWith(`${prefix}${item.href}/`)); return <Link key={item.href} href={`${prefix}${item.href}`} aria-current={active ? "page" : undefined} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? "bg-accent/10 font-semibold text-accent" : "text-muted hover:bg-[var(--surface-hover)] hover:text-foreground"}`}><span className={`flex h-7 w-7 items-center justify-center rounded-lg border text-base transition ${active ? "border-accent/30 bg-accent/10 text-accent" : "border-transparent text-accent group-hover:border-accent/30 group-hover:bg-accent/10"}`}>{item.icon}</span><span>{item.label}</span></Link>; })}</div></div>)}
    </nav>
